...
func TestAsynctimeout(t *testing.T) {
	...
	rep.tickDR()
	re.Equal(drStateSync, rep.drGetState()) // cannot switch state due to recently start
	...
	rep.tickDR()
	re.Equal(drStateSync, rep.drGetState()) // cannot switch state due to member not timeout
