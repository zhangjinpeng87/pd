...
func TestRegionStatistics(t *testing.T) {
	...
	regionStats.Observe(region1, stores)
	re.Len(regionStats.stats[ExtraPeer], 1)
	re.Len(regionStats.stats[MissPeer], 0)
	...
	re.Len(regionStats.offlineStats[ExtraPeer], 1)
	re.Len(regionStats.offlineStats[MissPeer], 0)
	...
	re.Len(regionStats.stats[ExtraPeer], 0)
	re.Len(regionStats.stats[MissPeer], 1)
	re.Len(regionStats.stats[DownPeer], 2)
	re.Len(regionStats.stats[PendingPeer], 1)
	re.Len(regionStats.stats[LearnerPeer], 0)
	re.Len(regionStats.offlineStats[ExtraPeer], 0)
	re.Len(regionStats.offlineStats[MissPeer], 0)
	re.Len(regionStats.offlineStats[DownPeer], 0)
	re.Len(regionStats.offlineStats[PendingPeer], 0)
	re.Len(regionStats.offlineStats[LearnerPeer], 0)
	re.Len(regionStats.offlineStats[OfflinePeer], 0)

	store3 = stores[3].Clone(core.UpStore())
	stores[3] = store3
	regionStats.Observe(region1, stores)
	re.Len(regionStats.stats[OfflinePeer], 0)
}
